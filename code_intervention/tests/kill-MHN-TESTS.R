...
##
## MHN has no cascade: killing gene X means removing X's row and column
## from the theta matrix. Every other gene's behaviour is unaffected except
## through the direct interactions with X. There is no AND/OR/XOR logic and
## no propagation to downstream nodes.
##
...
preds_from_mhn_theta <- function(th) {
  ...
  hpd <- tmp$MHN_hitting_probs_from_WT_direct
  div <- tmp$MHN_divergence_hitting_prob_calculation
