...
hesbcn_to_landscape_obj <- function(model, n_genes) {
  ...
  fitness_col <- ifelse("Birth" %in% colnames(genot_fitness),
                        "Birth", "Fitness")
...
preds_from_hesbcn_model <- function(x) {
  ...
  hpd <- tmp$HESBCN_hitting_probs_from_WT_direct
  div <- tmp$HESBCN_divergence_hitting_prob_calculation
...
test_that("HESBCN cascade AND: killing one AND-parent removes child and its descendants", {
  ...
  preds_from_model <- function(x) {
    ...
    hpd <- tmp[[paste0(method, "_hitting_probs_from_WT_direct")]]
    div <- tmp[[paste0(method, "_divergence_hitting_prob_calculation")]]
    list(genot_freqs = f[f > 0], hitting_probs_from_WT = filter_hp_keep_wt(hp),
         hitting_probs_from_WT_direct = filter_hp_keep_wt(hpd),
         divergence_hitting_prob_calculation = div)
  }
