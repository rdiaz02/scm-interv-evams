...
## This file tests the killing procedure of HyperHMM (similar to what
## kill-CBN/OT/OncoBN/HESBCN/MHN-TESTS.R do for other methods). Note that
## for HyperHMM, in contrast to other methods, there are no multiple
## equivalent ways of killing; see file kill-gene-equivalences-TESTS.R for
## CBN/OT/OncoBN/HESBCN/MHN equivalences.
...
##      And creating the transition rate matrix is also tested in
##      evamtools.
...
test_that("HyperHMM interventions and predicted genot calculations", {
  ...
  for (i in 1:reps) {
    ...
    local({
      hp <- h1_full$HyperHMM_hitting_probs_from_WT
    ...
    expect_equal(filter_hp_keep_wt(h1$HyperHMM_predicted_genotype_freqs),
                 o1[["no_intervention"]]$genot_freqs)
    ...
    expect_equal(filter_hp_keep_wt(pp[1, ]), ## we want a named vector
                 o1[["no_intervention"]]$genot_freqs)
...
test_that("HyperHMM", {
  ...
  drop_unreachable <- function(x) {
    rmr <- which(rowSums(x) == 0)
    x[-rmr, -rmr]
  }
  ...
  expect_true(isTRUE(all.equal(drop_unreachable(e_A),
                               hm1_A, check.attributes = FALSE)))
  ...
  expect_true(isTRUE(all.equal(drop_unreachable(e_B),
                               hm1_B, check.attributes = FALSE)))
  ...
  expect_true(isTRUE(all.equal(drop_unreachable(e_C),
                               hm1_C, check.attributes = FALSE)))
