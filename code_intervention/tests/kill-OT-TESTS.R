...
test_that("OT: compare predicted genotype freqs killing", {
  ...
  full_output_just_preds <- function(x) {
    ...
    hpd <- tmp$OT_hitting_probs_from_WT_direct
    div <- tmp$OT_divergence_hitting_prob_calculation
    return(list(genot_freqs = f[f > 0],
                hitting_probs_from_WT = filter_hp_keep_wt(hp),
